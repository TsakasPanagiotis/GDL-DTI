...
class FCNetworkPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('fc_network_2', 'template_1', 'experiments', 
...
class DiffusionNet(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        d_params, d_star_params, f, S0 = torch.split(output, [9, 9, 1, 1], dim=1)
        
        d_params = 7.5 * torch.tanh(d_params) + 2.5 # bounded in [-5, 10]
        A = d_params.reshape(-1, 3, 3)
        D = torch.bmm(A.transpose(1, 2), A)

        d_star_params = 75 * torch.tanh(d_star_params) + 25 # bounded in [-50, 100]
        A_star = d_star_params.reshape(-1, 3, 3)
        D_star = torch.bmm(A_star.transpose(1, 2), A_star)

        f = 0.65 * torch.sigmoid(f) + 0.05 #! bounded
...
def train(train_loader, model, loss_fn, optimizer, device, bvals, bvecs):
    ...
    for signals in train_loader:
        ...
        # pred_md = torch.linalg.eigvals(D).real.mean(dim=1)
        # pred_md_reg_term = torch.where(pred_md < md_low, (pred_md - md_low)**2, torch.tensor(0., device=pred_md.device))
        # pred_md_reg_term += torch.where(pred_md > md_high, (pred_md - md_high)**2, torch.tensor(0., device=pred_md.device))
        # pred_md_reg_term = pred_md_reg_term.mean()
        
        # pred_md_star = torch.linalg.eigvals(D_star).real.mean(dim=1)
        # pred_md_star_reg_term = torch.where(pred_md_star < md_star_low, (pred_md_star - md_star_low)**2, torch.tensor(0., device=pred_md_star.device))
        # pred_md_star_reg_term += torch.where(pred_md_star > md_star_high, (pred_md_star - md_star_high)**2, torch.tensor(0., device=pred_md_star.device))
        # pred_md_star_reg_term = pred_md_star_reg_term.mean()
        
        # batch_loss = loss_fn(recon_signals, signals) + pred_md_reg_term + pred_md_star_reg_term
