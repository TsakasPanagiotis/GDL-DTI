...
by approximating the lower triangular part 
of the diffusion tensor since it is symmetric.
...
class GroundTruthPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('ground_truth', 'template_2.3', 'experiments', 
...
def reconstruct(params: np.ndarray) -> tuple[np.ndarray, float]:
    ...
    D = np.array([[d_params[0], d_params[1], d_params[3]],
                  [d_params[1], d_params[2], d_params[4]],
                  [d_params[3], d_params[4], d_params[5]]])
