...
x_sin, x_cosine, y_sin, y_cosine, z_sin, z_cosine, eigval_1, eigval_2_over_1, eigval_3_over_2
and tanh or sigmoid to restrict the parameters.
...
@dataclass
class Paths:
    experiments_dir = os.path.join('ground_truth', 'template_7', 'experiments')
...
def reconstruct_tensor(params: np.ndarray, threshold_eigval: float) -> np.ndarray:
    ...
    x_sin, x_cosine, y_sin, y_cosine, z_sin, z_cosine, eigval_1, eigval_2_over_1, eigval_3_over_2 = params

    # Compute eigenvalues
    eigval_1 = sigmoid(eigval_1) * threshold_eigval
    eigval_2 = sigmoid(eigval_2_over_1) * eigval_1
    eigval_3 = sigmoid(eigval_3_over_2) * eigval_2

    # Activate sins and cosines
    x_sins = np.tanh(x_sin)
