...
x_sin, x_cos, y_sin, y_cos, z_sin, z_cos,
eigval_1, eigval_2_over_1, eigval_3_over_2.
Normalize sin and cos before using as sine and cosine.
...
class GroundTruthPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('ground_truth', 'template_9.1', 'experiments', 
...
def reconstruct(params: np.ndarray, threshold_eigval: float) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    x_sin, x_cos, y_sin, y_cos, z_sin, z_cos, eigval_1, eigval_2_over_1, eigval_3_over_2 = params

    norm_x = np.sqrt(x_sin**2 + x_cos**2)
    x_sin = x_sin / norm_x
    x_cos = x_cos / norm_x

    norm_y = np.sqrt(y_sin**2 + y_cos**2)
    y_sin = y_sin / norm_y
    y_cos = y_cos / norm_y

    norm_z = np.sqrt(z_sin**2 + z_cos**2)
    z_sin = z_sin / norm_z
    z_cos = z_cos / norm_z
