...
Get numpy linear least squares results.
...
@dataclass
class Paths:
    experiments_dir = os.path.join('ground_truth', 'template_1.1', 'experiments')
        ...
        for j in range(nii_data.shape[1]):
            for k in range(nii_data.shape[2]):
                if voxel_mask[i,j,k]:
                    ...
                    # avoid negative values in log function that follows
                    if np.any(S_over_S0 < 0.0):
                        pbar.update()
                        continue
                    ...
                    X = - b_values[selection_mask].reshape(-1,1) * \
                        np.array([g[:,0]**2, 2*g[:,0]*g[:,1], 2*g[:,0]*g[:,2], g[:,1]**2, 2*g[:,1]*g[:,2], g[:,2]**2]).T
