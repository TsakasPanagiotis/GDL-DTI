'''Based on ground truth template 9.1
Load simulated data.
...
x_cos, x_sin, y_cos, y_sin, z_cos, z_sin,
eigval_1, eigval_2_over_1, eigval_3_over_2.
Put (cos,sin) on unit circle before using as sine and cosine.
...
class BaselinePaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('baseline', 'template_7', 'experiments', 
...
def reconstruct(params: np.ndarray, threshold_eigval: float) \
    ...
    norm_x = np.sqrt(x_cos**2 + x_sin**2)
    x_cos = x_cos / norm_x
    x_sin = x_sin / norm_x

    # y angle should cover pi radians
    # angles in [-pi/2, pi/2] have cos(y) > 0
    y_cos = softplus(y_cos)

    norm_y = np.sqrt(y_cos**2 + y_sin**2)
    y_cos = y_cos / norm_y
    y_sin = y_sin / norm_y

    norm_z = np.sqrt(z_cos**2 + z_sin**2)
    z_cos = z_cos / norm_z
    z_sin = z_sin / norm_z
...
def main():
    ...
    for row_index in tqdm(range(sim_data.shape[0])):
        ...
        result = least_squares(
            loss, 
            params, 
            args=(S_norm, g, b, baseline_hparams.threshold_eigval),
            bounds=(min_bounds, max_bounds)
        )

        R, E, D, S0_correction = reconstruct(result.x, baseline_hparams.threshold_eigval)
    ...
    np.save(baseline_paths.eig_vecs_file, eig_vecs)
    np.save(baseline_paths.eig_vals_file, eig_vals)
