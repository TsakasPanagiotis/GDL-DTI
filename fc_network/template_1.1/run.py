...
x_angles, y_angles, z_angles, eigval_1, eigval_2_over_1, eigval_3_over_2
and sigmoid to restrict the parameters.
Note: y_angle restricted to [0, pi].
...
@dataclass
class Paths:
    experiments_dir = os.path.join('fc_network', 'template_1.1', 'experiments')
...
class SpectralDiffusionNet(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> torch.Tensor:
        ...
        x_angles, y_angles, z_angles, eig_val_1, eig_val_2_over_1, eig_val_3_over_2 = torch.split(output, split_size_or_sections=1, dim=1)      
        
        # Compute angles
        x_angles = x_angles.squeeze() * 2 * torch.pi
        y_angles = y_angles.squeeze() * torch.pi #! y_angles in [0, pi]
