...
Save checkpoints and losses plot for every 100 epochs.'''
...
class FCNetworkPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('fc_network', 'template_4.1', 'experiments', 
        ...
        self.checkpoint_file = os.path.join(self.experiment_path, 'checkpoint.pkl')
        self.model_template = os.path.join(self.experiment_path, 'model_{epoch}.pth')
...
def main():
    ...
    if resume_args.resume_experiment_paths_pkl:
        ...
        with open(fc_network_paths.checkpoint_file, 'rb') as f:
    ...
    for epoch in tqdm(range(last_epoch, last_epoch + fc_network_hparams.num_epochs)):
        ...
        if epoch % 100 == 0:
            ...
            with open(fc_network_paths.checkpoint_file, 'wb') as f:
                pickle.dump(checkpoint, f)

            model_file = fc_network_paths.model_template.format(epoch=epoch)
            
            with open(model_file, 'wb') as f:
                torch.save(diffusion_net, f)
            
            fc_network_paths.model_files[epoch] = model_file
