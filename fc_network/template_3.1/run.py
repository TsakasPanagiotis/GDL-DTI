...
that has eigenvectors and eigenvalues tensors instead of diffusion tensors.
...
MSE loss to compare ground truth diffusion tensors and reconstructed ones 
from the predicted eigenvectors and eigenvalues.
AdamW optimizer.
...
@dataclass
class Paths:
    experiments_dir = os.path.join('fc_network', 'template_3.1', 'experiments')
...
class SimulationDataPaths:
    ...
    @property
    def eigvecs_tensors_file(self):
        return os.path.join(self.simulation_data_experiment_path, 'eigvecs_tensors.pt')
    
    @property
    def eigvals_tensors_file(self):
        return os.path.join(self.simulation_data_experiment_path, 'eigvals_tensors.pt')
...
class DiffusionDataset(Dataset):
    ...
    def __init__(self, eig_vecs: torch.Tensor, eig_vals: torch.Tensor, noisy_signals: torch.Tensor):
        super().__init__()
    ...
    def __len__(self):
        return self.noisy_signals.shape[0]

    def __getitem__(self, idx):
        return self.eig_vecs[idx], self.eig_vals[idx], self.noisy_signals[idx]
...
class SpectralDiffusionNet(torch.nn.Module):
    ...
    def forward(self, input: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        # D = torch.bmm(eig_vecs, torch.bmm(eig_vals, eig_vecs.transpose(1, 2)))
...
class DiffusionLoss(torch.nn.Module):
    ...
    def forward(self, batch_eig_vecs: torch.Tensor, batch_eig_vals: torch.Tensor,
                pred_eig_vecs: torch.Tensor, pred_eig_vals: torch.Tensor) -> torch.Tensor:
        
        # both eigenvector tensors have shape (batch_size, 3, 3)
        # for every 3 by 3 matrix, the columns are the eigenvectors
        # we need to transpose to have the eigenvalues as rows
        # and then reshape to stack them in a two dimensional tensor
        batch_eig_vecs = batch_eig_vecs.transpose(1, 2).reshape(-1, 3)
        pred_eig_vecs = pred_eig_vecs.transpose(1, 2).reshape(-1, 3)

        # calculate the cosine similarity between the eigenvectors
        eig_vecs_loss = (1 - self.cos(batch_eig_vecs, pred_eig_vecs)).mean()

        # both eigenvalue tensors have shape (batch_size, 3, 3)
        # for every 3 by 3 matrix, the diagonal are the eigenvalues
        # we need to extract the diagonals in a two dimensional tensor
        batch_eig_vals = batch_eig_vals.diagonal(dim1=1, dim2=2)
        pred_eig_vals = pred_eig_vals.diagonal(dim1=1, dim2=2)

        # calculate the mean squared error between the eigenvalues
        eig_vals_loss = self.mse(batch_eig_vals, pred_eig_vals)

        return eig_vecs_loss + eig_vals_loss
...
def train(train_loader, model, loss_fn, optimizer, device) -> float:
    ...
    for batch_eig_vecs, batch_eig_vals, batch_noisy_signals in train_loader:
        
        batch_eig_vecs = batch_eig_vecs.to(device)
        batch_eig_vals = batch_eig_vals.to(device)
        batch_noisy_signals = batch_noisy_signals.to(device)

        batch_d_tensors = torch.bmm(batch_eig_vecs, torch.bmm(batch_eig_vals, batch_eig_vecs.transpose(1, 2))) #!
        ...
        # batch_loss = loss_fn(batch_eig_vecs, batch_eig_vals, pred_eig_vecs, pred_eig_vals)
...
def validate(val_loader, model, loss_fn, device) -> float:
    ...
    with torch.no_grad():
        ...
        for batch_eig_vecs, batch_eig_vals, batch_noisy_signals in val_loader:
...
def main() -> None:
    ...
    eig_vecs = torch.load(simulation_data_paths.eigvecs_tensors_file)
    eig_vals = torch.load(simulation_data_paths.eigvals_tensors_file)
    ...
    train_dataset = DiffusionDataset(eig_vecs[train_indices], eig_vals[train_indices], noisy_signals[train_indices])
    val_dataset = DiffusionDataset(eig_vecs[val_indices], eig_vals[val_indices], noisy_signals[val_indices])
    ...
    loss_fn = torch.nn.MSELoss() #! DiffusionLoss()
