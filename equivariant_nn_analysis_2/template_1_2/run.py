'''Connected to equivariant_nn_2 template 1.
...
Swap variables if D* has lower maximum eigenvalues than D.
...
from equivariant_nn_2.template_1.run import (
    get_selection_masks,
    SignalsDataset,
    SignalToIrreps,
    DiffusionDataset,
...
class EquivariantNNAnalysisPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('equivariant_nn_analysis_2', 'template_1_2', 'experiments', 
...
def main():
    ...
    with torch.no_grad():
        for signals, coeffs in test_loader:
            ...
            D_max_eigvals = torch.linalg.eigvals(D).real.max(dim=1)[0]
            D_star_max_eigvals = torch.linalg.eigvals(D_star).real.max(dim=1)[0]
            swap_mask = D_star_max_eigvals < D_max_eigvals
            D[swap_mask], D_star[swap_mask] = D_star[swap_mask], D[swap_mask]
            f[swap_mask] = 1 - f[swap_mask]
