...
of a supervised fc network approach.
...
class SupervisedFCNNHyperparameters(Protocol):
    simulation_data_paths_pkl: str
    threshold_eigval: float


class SupervisedFCNNPaths(Protocol):
    hyperparameters_file: str


class FCNetworkAnalysisHyperparameters(Protocol):
    supervised_fc_nn_paths_pkl: str
...
class EvaluationPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('evaluation', 'template_1.2', 'experiments', 
...
def main():
    ...
    with open(fc_network_analysis_hparams.supervised_fc_nn_paths_pkl, 'rb') as f:
