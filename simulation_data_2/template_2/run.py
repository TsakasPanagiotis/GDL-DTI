...
class SimulationDataPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('simulation_data_2', 'template_2', 'experiments', 
        ...
        self.f_values_histogram_file = os.path.join(self.experiment_path, 'f_values.png')
...
def get_noisy_biexp_signals(indices: np.ndarray, snrs: np.ndarray, d_tensors: np.ndarray, 
    ...
    f_values = np.random.beta(a=1.5, b=5.0, size=indices.shape[0]) + 0.05
...
def main():
    ...
    plt.hist(train_f_values, bins=100, label='Train')
    plt.hist(eval_f_values, bins=100, label='Eval')
    plt.hist(test_f_values, bins=100, label='Test')
    plt.legend()
    plt.xlabel('f-value')
    plt.ylabel('Count')
    plt.title('f-value Distribution')
    plt.savefig(simulation_data_paths.f_values_histogram_file)
    plt.close()
