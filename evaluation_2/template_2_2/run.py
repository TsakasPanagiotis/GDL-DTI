...
class EvaluationPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('evaluation_2', 'template_2_2', 'experiments', 
...
def main():
    ...
    with open(evaluation_hparams.baseline_paths_pkl, 'rb') as f:
    ...
    test_d_tensors = np.load(sim_data_paths.test_d_tensors_template.format(int(baseline_hparams.snr)))
    test_d_star_tensors = np.load(sim_data_paths.test_d_star_tensors_template.format(int(baseline_hparams.snr)))
    test_f_values = np.load(sim_data_paths.test_f_values_template.format(int(baseline_hparams.snr)))

    pred_d_tensors = np.load(baseline_paths.pred_d_tensors_file)
    pred_d_star_tensors = np.load(baseline_paths.pred_d_star_tensors_file)
    pred_f_values = np.load(baseline_paths.pred_f_values_file)
