...
from a equivariant_nn_analysis experiment.
...
class EquivariantNNAnalysisHyperparameters(Protocol):
    equivariant_nn_paths_pkl: str
    snr: float


class EquivariantNNAnalysisPaths(Protocol):
    hyperparameters_file: str
...
class EvaluationHyperparameters:
    equivariant_nn_analysis_paths_pkl: str


class EvaluationPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('evaluation_2', 'template_2_3', 'experiments', 
...
def main():
    ...
    parser.add_argument('--equivariant_nn_analysis_paths_pkl', type=str, required=True)
    ...
    with open(evaluation_hparams.equivariant_nn_analysis_paths_pkl, 'rb') as f:
        equivariant_nn_analysis_paths: EquivariantNNAnalysisPaths = pickle.load(f)
    

    ## EQUIVARIANT NN ANALYSIS HYPERPARAMETERS

    with open(equivariant_nn_analysis_paths.hyperparameters_file, 'rb') as f:
        equivariant_nn_analysis_hparams: EquivariantNNAnalysisHyperparameters = pickle.load(f)
    ...
    test_d_tensors = np.load(sim_data_paths.test_d_tensors_template.format(int(equivariant_nn_analysis_hparams.snr)))
    test_d_star_tensors = np.load(sim_data_paths.test_d_star_tensors_template.format(int(equivariant_nn_analysis_hparams.snr)))
    test_f_values = np.load(sim_data_paths.test_f_values_template.format(int(equivariant_nn_analysis_hparams.snr)))

    pred_d_tensors = np.load(equivariant_nn_analysis_paths.pred_d_tensors_file)
    pred_d_star_tensors = np.load(equivariant_nn_analysis_paths.pred_d_star_tensors_file)
    pred_f_values = np.load(equivariant_nn_analysis_paths.pred_f_values_file).squeeze()
