...
class SimulationDataPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('simulation_data', 'template_4.1', 'experiments', 
...
def main():
    ...
    for test_snr in simulation_data_hparams.test_snrs:
        ...
        noisy_test_signals = get_noisy_signals(test_indices, test_snrs, d_tensors, b_values, b_vectors)

        logging.info(f'Test data for SNR = {test_snr}: min = {noisy_test_signals.min()} - max = {noisy_test_signals.max()}')

        np.save(simulation_data_paths.test_data_template.format(test_snr), noisy_test_signals)
