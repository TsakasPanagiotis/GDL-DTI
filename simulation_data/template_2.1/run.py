'''Load b-values, direction vectors.
Load least squares results from given ground truth experiment
that contains eigenvectors and eigenvalues estimations.
Use selected b-values to create a selection mask.
Produce noisy signals with given SNR.
Store all as torch tensors.'''
...
@dataclass
class Paths:
    experiments_dir = os.path.join('simulation_data', 'template_2.1', 'experiments')
    ...
    def eigvecs_tensors_file(self):
        return os.path.join(self.experiment_path, 'eigvecs_tensors.pt')
    
    @property
    def eigvals_tensors_file(self):
        return os.path.join(self.experiment_path, 'eigvals_tensors.pt')
...
class GroundTruthPaths:
    ...
    @property
    def ground_truth_hyperparameters_file(self):
        return os.path.join(self.ground_truth_experiment_path, 'hyperparameters.pkl')
    
    @property
    def lstsq_results_path(self):
        return os.path.join(self.ground_truth_experiment_path, 'lstsq_results.pkl')
...
def main():
    ...
    parser.add_argument('--ground_truth_experiment_path', type=str, required=True)
    ...
    ground_truth_paths = GroundTruthPaths(HP.ground_truth_experiment_path)

    with open(ground_truth_paths.ground_truth_hyperparameters_file, 'rb') as f:
    ...
    with open(ground_truth_paths.lstsq_results_path, 'rb') as f:
        lstsq_results: dict[tuple[int,int,int], tuple[np.ndarray, np.ndarray, np.ndarray]] = pickle.load(f)
    ...
    for eigvecs_matrix, eigvals_matrix, d_array in tqdm(lstsq_results.values()):

        eigvecs_tensor = torch.tensor(eigvecs_matrix).float()
        eigvals_tensor = torch.tensor(eigvals_matrix).float()

        d_tensor_rec = eigvecs_tensor @ eigvals_tensor @ eigvecs_tensor.T
        d_tensor = torch.tensor(d_array).float() #!
        
        signal_rec = torch.exp(- selected_b_values * torch.einsum('bi, ij, bj -> b', selected_gradients, d_tensor_rec, selected_gradients))
        signal = torch.exp(- selected_b_values * torch.einsum('bi, ij, bj -> b', selected_gradients, d_tensor, selected_gradients))

        assert torch.allclose(signal_rec, signal)
        ...
        eigvecs_tensors.append(eigvecs_tensor)
        eigvals_tensors.append(eigvals_tensor)
        d_tensors.append(d_tensor) #!
        noisy_signals.append(noisy_signal)

    eigvecs_tensors = torch.stack(eigvecs_tensors) # shape (num_d_tensors, 3, 3)
    eigvals_tensors = torch.stack(eigvals_tensors) # shape (num_d_tensors, 3)
    ...
    logging.info(f'eigvecs_tensors: {eigvecs_tensors.shape}')
    logging.info(f'eigvals_tensors: {eigvals_tensors.shape}')
    ...
    torch.save(eigvecs_tensors, paths.eigvecs_tensors_file)
    torch.save(eigvals_tensors, paths.eigvals_tensors_file)
