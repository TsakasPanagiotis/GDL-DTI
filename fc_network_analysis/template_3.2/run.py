'''Connected to fc_network template 4.1.
...
Pass through the all saved models.
Save diffusion tensors for each model.
Measure reconstruction errors for each model.'''
...
class FCNetworkAnalysisPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('fc_network_analysis', 'template_3.2', 'experiments', 
        ...
        self.pred_d_tensors_template = os.path.join(self.experiment_path, 'pred_d_tensors_{epoch}.npy')
        self.pred_d_tensors_files: dict[int, str] = {}
...
def main():
    ...
    for epoch, model_file in fc_network_paths.model_files.items():

        ## LOAD MODEL
        
        model: DiffusionNet = torch.load(model_file).to(device)
        ...
        pred_d_tensors_file = fc_network_analysis_paths.pred_d_tensors_template.format(epoch=epoch)
        fc_network_analysis_paths.pred_d_tensors_files[epoch] = pred_d_tensors_file
        np.save(pred_d_tensors_file, pred_d_tensors.cpu().numpy())
