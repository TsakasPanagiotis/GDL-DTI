...
Load simulated data.
Load baseline d-tensors, S0 corrections and valid indices.
...
class BaselineHyperparameters(Protocol):
    b_values_to_select: list[float]
    simulation_data_paths_pkl: str


class BaselinePaths(Protocol):
    hyperparameters_file: str
    d_tensors_file: str
...
@dataclass
class BaselineAnalysisHyperparameters:
    baseline_paths_pkl: str


class BaselineAnalysisPaths:
    def __init__(self) -> None:
        self.experiment_path = os.path.join('baseline_analysis', 'template_1', 'experiments', 
...
def main():
    ...
    ## BASELINE ANALYSIS PATHS

    baseline_analysis_paths = BaselineAnalysisPaths()

    print(f'Experiment path: {baseline_analysis_paths.experiment_path}')

    os.makedirs(baseline_analysis_paths.experiment_path)

    with open(baseline_analysis_paths.paths_file, 'wb') as f:
        pickle.dump(baseline_analysis_paths, f)
    

    ## LOGGING

    logging.basicConfig(
        filename=baseline_analysis_paths.log_file,
        level=logging.INFO,
        format='%(asctime)s - %(levelname)s - %(message)s')
    
    logging.info(f'Baseline analysis experiment:')
    logging.info(baseline_analysis_paths.experiment_path)
    logging.info('')
    

    ## BASELINE ANALYSIS HYPERPARAMETERS

    parser = ArgumentParser()
    parser.add_argument('--baseline_paths_pkl', type=str, required=True)
    args = parser.parse_args()

    baseline_analysis_hparams = BaselineAnalysisHyperparameters(**vars(args))

    with open(baseline_analysis_paths.hyperparameters_file, 'wb') as f:
        pickle.dump(baseline_analysis_hparams, f)    
    
    logging.info('Baseline analysis hyperparameters:')
    for key, value in vars(baseline_analysis_hparams).items():
    ...
    with open(baseline_analysis_hparams.baseline_paths_pkl, 'rb') as f:
        baseline_paths: BaselinePaths = pickle.load(f)
    

    ## BASELINE HYPERPARAMETERS

    with open(baseline_paths.hyperparameters_file, 'rb') as f:
        baseline_hparams: BaselineHyperparameters = pickle.load(f)
    ...
    d_tensors: np.ndarray = np.load(baseline_paths.d_tensors_file)
    S0_corrections: np.ndarray = np.load(baseline_paths.S0_corrections_file)
    valid_indices: np.ndarray = np.load(baseline_paths.valid_indices_file)
    ...
    valid_data = sim_data[valid_indices]
